...
Figure~\ref{fig:opt-vs-emp} overlays the model-implied distribution of optimal call times against the empirical NHL distribution from the 468-call corpus. The implied distribution is generated by Monte Carlo simulation of 5{,}000 game trajectories starting from a tied score at $t = 0$, with the optimal-stopping rule applied. The two distributions barely overlap. Optimal mass concentrates at minute 48 to 50; observed mass concentrates at minute 58 to 60.
...
We framed the NHL timeout decision as a real option, solved the optimal-stopping problem numerically using empirically-calibrated league scoring dynamics, and compared the implied optimal exercise distribution to the observed distribution from three regular seasons of play-by-play data. The optimal policy uses the timeout in 58 to 86 percent of team-games at a median call two to eleven minutes earlier than current practice. The observed usage rate is 5.95 percent. The empirical policy captures approximately 11 percent of the available value of the timeout option. The dominant component of the EU loss is the option-decay term: 94 percent of held timeouts expire worthless.
